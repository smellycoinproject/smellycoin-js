...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.post('/operator/wallet/verify', (req, res, next) => {
            ...
            try {
                if (!operator.checkWalletPassword(walletId, passwordHash)) throw new HTTPError(403, `False`);
        ...
        this.app.post('/miner/mine', (req, res, next) => {
            var senderip = req.socket.remoteAddress;
            if (senderip != '127.0.0.1'){
                res.send('Only local addresses can mine using the api!');
                return;
            }
